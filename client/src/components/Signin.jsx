...
const UserTypeButton = styled.button`
  ...
  border: none;
  border-radius: 20px;
  background-color: ${({ active, theme }) => active ? theme.primary : theme.button};
...
const SignIn = ({ setSignInOpen, setSignUpOpen }) => {
  ...
  const handleLogin = async (e) => {
    ...
    if (!disabled) {
      ...
      setLoading(true);
      try {
        const res = await signIn({ email, password, userType });
        if (res.status === 200) {
        ...
        setLoading(false);
        setDisabled(false);
        setcredentialError(`Invalid Credentials: ${err.message}`);
        dispatch(
          openSnackbar({
            message: err.message,
            severity: "error",
